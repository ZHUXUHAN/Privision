...
manager = Manager()
#
task1_in = manager.Queue(200)
task1_out = manager.Queue(200)
task2_in = manager.Queue(200)
task2_out = manager.Queue(200)
task3_in = manager.Queue(200)
task3_out = manager.Queue(200)
task4_in = manager.Queue(200)
task4_out = manager.Queue(200)
...
class MainWindow(QMainWindow):
    ...
    def onNewImage(self, frame):
        ...
        t = clock()
        if cfg_priv.GLOBAL.F_MODEL1 or cfg_priv.GLOBAL.F_MODEL2 or cfg_priv.GLOBAL.F_MODEL3:
            vis = Worker01(frame,
                           show_box=cfg_priv.GLOBAL.F_MODEL1,
                           show_kpts=cfg_priv.GLOBAL.F_MODEL2,
                           show_mask=cfg_priv.GLOBAL.F_MODEL3)
        ...
        if cfg_priv.GLOBAL.F_MODEL1:
            task1_in.put(frame)
        if cfg_priv.GLOBAL.F_MODEL2:
            task2_in.put(frame)
        if cfg_priv.GLOBAL.F_MODEL3:
            task3_in.put(frame)
        if cfg_priv.GLOBAL.F_MODEL4:
            task4_in.put(frame)
        while True:
            if cfg_priv.GLOBAL.F_MODEL1:
