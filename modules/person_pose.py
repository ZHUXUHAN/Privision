...
import pet.maskrcnn.nn as mynn
import pet.maskrcnn.datasets.dummy_datasets as datasets
import pet.maskrcnn.utils.net as net_utils
import pet.maskrcnn.utils.vis as vis_utils
from pet.maskrcnn.core.config import cfg, merge_cfg_from_file, assert_and_infer_cfg
from pet.maskrcnn.core.test import im_detect_all
from pet.maskrcnn.modeling.model_builder import Generalized_RCNN
from pet.maskrcnn.utils.detectron_weight_helper import load_detectron_weight
from pet.maskrcnn.utils.timer import Timer

cv2.ocl.setUseOpenCL(False)

merge_cfg_from_file(osp.join(cfg_priv.GLOBAL.PYTORCH_EVERYTHING_ROOT, cfg_priv.MODULES.PPO.CFG))
# pprint.pprint(cfg)
...
def get_weights(mode='final'):
    if os.path.exists(cfg.TEST.WEIGHTS):
        ...
        model_root = os.path.dirname(cfg_priv.MODULES.PPO.CFG)
        # pkl first
