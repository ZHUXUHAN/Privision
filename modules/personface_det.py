...
from .face_keypoints import FaceKpts, vis_kpts
...
import pet4.maskrcnn.nn as mynn
import pet4.maskrcnn.datasets.dummy_datasets as datasets
import pet4.maskrcnn.utils.net as net_utils
import pet4.maskrcnn.utils.vis as vis_utils
from pet4.maskrcnn.core.config import cfg, merge_cfg_from_file, assert_and_infer_cfg
from pet4.maskrcnn.core.test import im_detect_all
from pet4.maskrcnn.modeling.model_builder import Generalized_RCNN
from pet4.maskrcnn.utils.detectron_weight_helper import load_detectron_weight
from pet4.maskrcnn.utils.timer import Timer

cv2.ocl.setUseOpenCL(False)

merge_cfg_from_file(osp.join(cfg_priv.GLOBAL.PYTORCH_EVERYTHING_ROOT, cfg_priv.MODULES.PFD.CFG))
pprint.pprint(cfg)
...
class PersonFaceDet:
    def __init__(self, gpu_id=0):
        ...
        self.face_kpts = FaceKpts(gpu_id)
    ...
    def __call__(self, img, show_box=True, show_kpts=True, show_mask=True):
        ...
        if len(cls_boxes_i[1]) > 0:
            kpts = self.face_kpts(img, cls_boxes_i, face_idx=1, min_thresh=0.8, min_abso_area=50 * 50,
                                  min_rela_area=0.005, max_aspect_ratio=2.4)
            if kpts is not None:
                vis_kpts(vis_im, kpts)
...
def get_weights(mode='final'):
    if os.path.exists(cfg.TEST.WEIGHTS):
        ...
        model_root = os.path.dirname(cfg_priv.MODULES.PFD.CFG)
        # pkl first
